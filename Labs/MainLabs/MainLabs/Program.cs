...
{
	static class calculatorOfSpecificExpressions
	{
		static void Main()
		{
		...
		static float EvaluateExpression(string inputData)
		{
			...
			for (int i = 0; i < inputData.Length; i++)
			{
				...
				else if (IsOperator(currentChar))
				{
					...
					{
						PerformanceOfCalculation(operators, numbers);
					}
		...
		public static void PerformanceOfCalculation(List<(char Operator, int Priority)> operators, List<float> operands)
		{
			...
			operands.Add(PerformanceOfOperation(op, operand1, operand2));
		}

		static float PerformanceOfOperation(char op, float operand1, float operand2)
		{
			switch (op)
			{
